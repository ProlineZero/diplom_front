...
export function CarPage() {
  ...
  async function fetchCar() {
    const response = await axios.get<ICar>('http://192.168.1.11/api/cars-detail/' + String(id))
  ...
  async function checkIsLiked() {
    if (localStorage.getItem('jwt')) {
      const response = await axios.post<{success:boolean}>('http://192.168.1.11/api/is-car-in-favorites/', {user_jwt: localStorage.getItem('jwt'), car_id: id})
  ...
  function set_to_favorites(btnLikeStyle: string) {
    if (localStorage.getItem('jwt')) {
      if (btnLikeStyle == btnNotLiked) {
        setIsLiked(true)
        axios.post('http://192.168.1.11/api/add-to-favorites/', {user_jwt: localStorage.getItem('jwt'), car_id: id})
      } else {
        setIsLiked(false)
        axios.post('http://192.168.1.11/api/delete-from-favorites/', {user_jwt: localStorage.getItem('jwt'), car_id: id})
      }
    } else {
      setPromtAutho(true)
  ...
  const btnLiked = 'absolute left-[90%] -translate-x-1/2 top-[90%] -translate-y-1/2 font-medium bg-gray-100 rounded-full text-green-700'
  const btnNotLiked = 'absolute left-[90%] -translate-x-1/2 top-[90%] -translate-y-1/2 font-medium text-green-300 bg-gray-100 rounded-full'
  ...
  return (
    <>
      ...
      <div className= "fixed bg-gray-100 w-full h-[60%] rounded-3xl border-l-2 border-r-2 border-green-600 shadow-xl shadow-black/50">
      ...
      <div className= "fixed bg-gray-100 w-full h-[32%] top-[68%] rounded-3xl border-l-2 border-r-2 border-green-600 shadow-xl shadow-black/50">
      <div className='absolute grid gap-y-2 gap-x-3 grid-cols-5 grid-rows-4 w-full h-full  items-center py-4 px-10'>

      <div className="block p-2 text-sm md:text-base md:leading-none xl:text-base 2xl:text-lg 2xl:leading-none xl:leading-none leading-none font-medium text-center border-2 border-t-0 border-l-0 rounded-t-none rounded-l-none rounded-3xl border-green-500/50">
