...
export function AuthorizationPage() {
  ...
  async function checkAuthorization()  {
    try {
    await axios.post<{jwt: string}>('http://192.168.1.11/api/login/', user).catch(error => {setAutStatus(false)})
    const response = await axios.post<{jwt: string}>('http://192.168.1.11/api/login/', user)
  ...
  return (
    <>
      ...
      <div
        className = "fixed bg-gray-100 w-5/12 h-1/2 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 rounded-3xl border-l-2 border-r-2 border-green-600 shadow-2xl shadow-black/50">
        ...
        <div className="container w-1/2 space-y-3 fixed inline-block text-left left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2">
          ...
          <h1 className='text-gray-500 text-sm text-center overline'>Нет аккаунта в Guide2Cars?</h1>
          ...
          <div className={errorAutho? 'visible' : 'hidden'}>
            <h1 className='text-green-500 text-xm text-center py-2 underline'>Неправильно указан логин и/или пароль</h1>
          </div>
          <button type="button" className="w-full px-2 py-1 transition ease-in duration-200 uppercase rounded-full bg-green-100 text-green-700 border-green-100 hover:bg-green-200 hover:border-green-200 border-2 focus:outline-none "
